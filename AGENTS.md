...
- `console/` - Core server package
  ...
  - `templates/` - DeploymentTemplateService managing CUE-based deployment templates stored as K8s ConfigMaps; embeds `default_template.cue`. Templates use the structured `output.namespacedResources`/`output.clusterResources` output format (see `docs/cue-template-guide.md`). Templates can carry `DeploymentDefaults` (image, tag, command, args, env, port) that pre-fill the Create Deployment form. The `RenderDeploymentTemplate` RPC returns rendered resources as both YAML (`rendered_yaml`) and JSON (`rendered_json`). The default template adds a `console.holos.run/deployer-email` annotation to all resources from `system.claims.email`. The default template includes a `ReferenceGrant` (using `system.gatewayNamespace`, default "istio-ingress") that allows HTTPRoute resources from the gateway namespace to reference Services in the project namespace.
  - `system_templates/` - SystemTemplateService managing org-scoped CUE templates stored as K8s ConfigMaps in the org namespace. System templates use `package deployment` (same CUE package as deployment templates) and are unified with the deployment template at deploy time so they can reference `input.*` and `system.*` fields. Templates can be marked `mandatory` (applied to project namespaces at creation time) and/or `enabled` (unified at deploy time). `MandatoryTemplateApplier` applies only mandatory AND enabled templates at project creation. Embeds `default_referencegrant.cue` as the built-in example HTTPRoute system template (seeded as disabled, not mandatory). Edit access requires `PERMISSION_SYSTEM_DEPLOYMENTS_EDIT` (org-level OWNER only via `OrgCascadeSystemTemplatePerms`).
  - `deployments/` - DeploymentService managing Kubernetes Deployments: CRUD, status polling, log streaming, CUE render and apply (structured `output.namespacedResources`/`output.clusterResources` output), container command/args override, container env vars (literal values, SecretKeyRef, ConfigMapKeyRef), container port configuration, listing project-namespace Secrets/ConfigMaps for env var references, and `GetDeploymentRenderPreview` (returns the CUE template, system input, user input, rendered YAML, and rendered JSON for a live deployment). CUE render uses split `SystemInput` (project, namespace, gatewayNamespace, claims) and `UserInput` (name, image, tag, etc.) — see `docs/cue-template-guide.md`. At render time, enabled system templates for the project's org are loaded and unified with the deployment template via `CueRenderer.RenderWithSystemTemplates`; system templates contribute resources to `output.systemNamespacedResources` and `output.systemClusterResources`.
