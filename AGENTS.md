...
Authorization is enforced by Kubernetes RBAC with OIDC impersonation per [ADR 036](docs/adrs/036-rbac-and-oidc-impersonation.md). Every ConnectRPC handler that calls Kubernetes routes the request through an impersonated clientset so the API server — not in-process Go code — is the single arbiter of access. As of HOL-1066 the legacy `console/rbac` package has been trimmed to a residue: `console/settings` is the last in-process call site of `rbac.CheckAccessGrants`/`rbac.CheckCascadeAccess`, and `console/{organizations,folders,projects}` use the residual Role enum + `BestRoleFromGrants` only to derive the `userRole` field returned in responses (UI hint, not access gate). New handlers MUST use `rpc.ImpersonatedClientsetFromContext(ctx)` and MUST NOT call `rbac.CheckAccessGrants` or `rbac.CheckCascadeAccess`. Per-resource RoleBindings are reconciled by `console/resourcerbac` (HOL-1062 / HOL-1063); see [ADR 036](docs/adrs/036-rbac-and-oidc-impersonation.md) and the migration runbook in `cmd/holos-console-migrate-rbac/` for context.
