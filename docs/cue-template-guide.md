...
| `console/templates/k8s.go` | ConfigMap storage: templates stored with `template.cue` data key, `console.holos.run/template-scope` label (`organization|folder|project`), `mandatory` and `enabled` annotations. `ListAncestorTemplateSourcesForRender` walks the full ancestor chain (org + folders) applying the render set formula `(mandatory AND enabled) UNION (enabled AND ref.Name IN linkedRefs)` at each scope. `ListOrgTemplateSourcesForRender` provides the same formula for org-scope-only resolution (used by the template handler fallback). |
