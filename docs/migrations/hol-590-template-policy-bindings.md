...
On `--apply` the migrator rewrites every rule's `target` to the empty struct `{project_pattern: "", deployment_pattern: ""}` — the shape the ticket's AC specifies. HOL-600 has since removed the proto-level `TemplatePolicyTarget` message and the resolver's legacy glob evaluation path, so:

1. New `TemplatePolicy` writes no longer emit the `target` JSON field at all; stored ConfigMaps still carrying the legacy payload decode it defensively and drop the value. UI/API updates of a migrated policy are accepted (the HOL-598 → HOL-600 transition window where the validator rejected empty patterns is closed).
2. A `"*"` placeholder was never used for the clear step, both because the resolver only bypassed a rule's glob for render targets a matching binding specifically covered — broadening the policy's effective set to every render target not named by the binding (new projects, new deployments) — and because `"*"` is a legitimate pre-migration wildcard, so the idempotency classifier would have been unable to tell a legacy wildcard rule apart from a migrator-cleared rule, silently skipping policies that actually still need translation.
