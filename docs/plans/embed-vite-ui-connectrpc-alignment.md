...
2. Switch the `/ui` redirect status to HTTP 301 in the Go server (`console/console.go`) and Vite dev server plugin (`ui/vite.config.ts`) so the canonical URL is `/ui/`.
3. Re-check server routing to confirm `/ui/` routes are handled by the UI handler and ConnectRPC endpoints remain unaffected (no code change expected).
4. (Optional) Run the UI unit tests to ensure router changes still pass.
