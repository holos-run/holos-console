...
// rewrite. K8sClient now reads and writes TemplatePolicyBinding CRDs
// through a controller-runtime client.Client; tests construct either a
// fake ctrlclient (for the handler-level suite that still speaks in terms
// of CRD fixtures) or a cache-backed envtest client (for k8s_test.go).
...
// v1alpha1 so the client accepts TemplatePolicyBinding CRs.
