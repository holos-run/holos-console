...
// checkAccess enforces the TemplatePolicyBindingPerms RBAC table on the
// binding's owning scope. Project scope is rejected up front; the table exists
// only at org/folder, so any attempt to check a project-scope binding here is
// a programming error that earlier handler validation should have caught.
...
func (h *Handler) checkOrgAccess(ctx context.Context, claims *rpc.Claims, org string, perm rbac.Permission) error {
	...
	return rbac.CheckCascadeAccess(claims.Email, claims.Roles, users, roles, perm, rbac.TemplatePolicyBindingPerms)
}
