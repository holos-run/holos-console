...
//
// Post-HOL-723 the getter speaks in Kubernetes namespace directly; the
// (scope, scopeName) pair has been retired along with the scopeshim
// compatibility layer.
...
// getter. Post-HOL-723 the adapter no longer needs a resolver because it
// speaks in Kubernetes namespace directly; the parameter is kept optional
// (accepted and ignored) so existing call sites compile without churn.
func NewPolicyExistsAdapter(g PolicyExistsGetter, _ *resolver.Resolver) *PolicyExistsAdapter {
	return &PolicyExistsAdapter{Getter: g}
}

// PolicyExists reports whether a TemplatePolicy with the given namespace
// and name exists. Returns (false, nil) for K8s NotFound so the handler can
// cleanly reject with CodeInvalidArgument.
func (a *PolicyExistsAdapter) PolicyExists(ctx context.Context, namespace, name string) (bool, error) {
	if a == nil || a.Getter == nil {
		return false, fmt.Errorf("policy-exists adapter is not configured")
	}
	_, err := a.Getter.GetPolicy(ctx, namespace, name)
	if err == nil {
		return true, nil
	}
...
// failures. The parentNamespace parameter is accepted for interface
// compatibility but not consumed — the resolver's prefix scheme already
// pins the project namespace from the slug alone, and authoring-time
// existence is scope-independent per HOL-595.
func (a *ProjectExistsAdapter) ProjectExists(ctx context.Context, _ string, projectName string) (bool, error) {
