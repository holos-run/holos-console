...
func TestTemplateDependencyAccess(t *testing.T) {
	tests := []struct {
		name    string
		email   string
	...
	}{
		{
			...
			run: func(h *Handler, ns string, ctx context.Context) error {
				_, err := h.ListTemplateDependencies(ctx, connect.NewRequest(&consolev1.ListTemplateDependenciesRequest{Namespace: ns}))
				return err
			},
		},
		{
			...
			grants: map[string]string{"viewer@example.com": "viewer"},
			run: func(h *Handler, ns string, ctx context.Context) error {
				_, err := h.CreateTemplateDependency(ctx, connect.NewRequest(&consolev1.CreateTemplateDependencyRequest{
					Namespace:  ns,
					Dependency: basicDependency(ns),
				}))
				return err
		...
		},
		{
			...
			seedObj: true,
			run: func(h *Handler, ns string, ctx context.Context) error {
				_, err := h.DeleteTemplateDependency(ctx, connect.NewRequest(&consolev1.DeleteTemplateDependencyRequest{
					Namespace: ns,
					Name:      "api-needs-gateway",
				}))
				return err
		...
		},
		{
			...
			run: func(h *Handler, ns string, _ context.Context) error {
				_, err := h.ListTemplateDependencies(context.Background(), connect.NewRequest(&consolev1.ListTemplateDependenciesRequest{Namespace: ns}))
				return err
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.seedObj {
				objs = append(objs, &templatesv1alpha1.TemplateDependency{
					ObjectMeta: metav1.ObjectMeta{Name: "api-needs-gateway", Namespace: newTestResolver().ProjectNamespace("checkout")},
					Spec: templatesv1alpha1.TemplateDependencySpec{
						Dependent: templatesv1alpha1.LinkedTemplateRef{Namespace: newTestResolver().ProjectNamespace("checkout"), Name: "api"},
						Requires:  templatesv1alpha1.LinkedTemplateRef{Namespace: "holos-org-acme", Name: "gateway"},
					},
				})
			}
