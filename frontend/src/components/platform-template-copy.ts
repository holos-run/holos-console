 ...
 *
 * Platform templates describe eligibility and render-time inclusion via
 * TemplatePolicyBinding rules; they do not directly "apply" resources to
 * project namespaces. Drift between the org and folder wordings previously
 * claimed the opposite, which was the root cause of the misdescription bug
 * tracked under HOL-580. Consolidating the copy here prevents the wordings
 * from diverging again.
 *
 * The `enabled` flag only controls render-time unification eligibility.
...
export const ENABLED_TOGGLE_ACTIVE_DESCRIPTION =
  'Enabled — eligible to be included when rendering downstream templates and deployments via TemplatePolicyBinding rules.'

export const ENABLED_TOGGLE_INACTIVE_DESCRIPTION =
  'Disabled — excluded from render-time unification. Existing TemplatePolicyBinding rules that reference this template render as no-ops.'
...
export const EXCLUDE_RULE_DESCRIPTION =
  'EXCLUDE — when a project template or deployment matching the target is rendered, remove this platform template from the effective ref set.'
